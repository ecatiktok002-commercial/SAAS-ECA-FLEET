...
interface EditAchievementModalProps {
  ...
  onSave: (id: string, icon: string, title: string, desc: string, trigger: string, taskReq?: string, triggerVal?: number) => void;
}

export default function EditAchievementModal({ isOpen, onClose, achievement, onSave }: EditAchievementModalProps) {
  ...
  useEffect(() => {
    if (achievement) {
      setIcon(achievement.icon || 'military_tech');
      setTitle(achievement.title || '');
      setDesc(achievement.desc || '');
      setTrigger(achievement.trigger || 'TASK_COMPLETED');
      setTaskReq(achievement.taskRequired || '');
      setTriggerVal(achievement.triggerValue || 1);
    }
  }, [achievement]);
  ...
  const handleSubmit = () => {
    ...
    onSave(achievement.id, icon, title, desc, trigger, taskReq || undefined, triggerVal);
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
      ...
      >
        <div className="flex justify-between items-start mb-8">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-secondary mb-1">Edit Registry</p>
            <h3 className="text-3xl font-extrabold font-headline text-on-surface tracking-tight">Modify Milestone</h3>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="max-h-[60vh] overflow-y-auto no-scrollbar space-y-6">
          <div className="grid grid-cols-1 gap-5">
              ...
              <div className="relative">
                <select 
                  ...
                  onChange={e => {
                    ...
                    // Reset fields when changing trigger
                    setTaskReq('');
                    setTriggerVal(1);
                  }} 
                ...
                >
                  <option value="TASK_COMPLETED">Specific Task Completion</option>
                  <option value="TIER_MASTERY">Complexity Tier Mastery</option>
                  <option value="POINTS_MILESTONE">Total Points Milestone</option>
                  <option value="LOGIN_STREAK">Consecutive Daily Streak</option>
                  <option value="MODULE_CERTIFIED">Training Module Completed</option>
                  <option value="MANUAL">Manual Award Only</option>
                </select>
            ...
            <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
              {(trigger === 'TASK_COMPLETED' || trigger === 'MODULE_CERTIFIED') && (
                <div className={trigger === 'MODULE_CERTIFIED' ? "col-span-2" : ""}>
                  <label className="text-[10px] font-bold uppercase tracking-widest block mb-2 text-primary ml-1">
                    {trigger === 'TASK_COMPLETED' ? 'Required Task Name' : 'Module Code / Name'}
                  </label>
                  <input 
                    value={taskReq} 
                    ...
                    placeholder={trigger === 'TASK_COMPLETED' ? "E.g., Weekly Report" : "E.g., MOD-001"} 
                  />
                </div>
              )}

              {trigger === 'TIER_MASTERY' && (
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest block mb-2 text-primary ml-1">Target Tier</label>
                  <select 
                    value={taskReq} 
                    onChange={e => setTaskReq(e.target.value)} 
                    className="w-full bg-surface-container rounded-2xl py-4 px-5 outline-none border border-outline-variant/5 text-on-surface font-bold focus:border-primary/30 focus:ring-4 focus:ring-primary/5 transition-all shadow-inner appearance-none"
                  >
                    <option value="" disabled>Select Tier</option>
                    <option value="Tier 1">Tier 1: Routine</option>
                    <option value="Tier 2">Tier 2: Standard</option>
                    <option value="Tier 3">Tier 3: Complex</option>
                    <option value="Tier 4">Tier 4: Critical</option>
                    <option value="Tier 5">Tier 5: Extraordinary</option>
                  </select>
                </div>
              )}

              {trigger !== 'MODULE_CERTIFIED' && trigger !== 'MANUAL' && (
                <div className={trigger === 'POINTS_MILESTONE' || trigger === 'LOGIN_STREAK' ? "col-span-2" : ""}>
                  <label className="text-[10px] font-bold uppercase tracking-widest block mb-2 text-primary ml-1">
                    {trigger === 'POINTS_MILESTONE' ? 'Points Required' : 
                     trigger === 'LOGIN_STREAK' ? 'Consecutive Days' : 'Times Completed'}
                  </label>
                  <input 
                    type="number"
                    min="1"
