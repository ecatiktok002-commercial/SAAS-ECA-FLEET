...
interface ProfileModalProps {
  ...
  onDeleteAvatar?: (url: string) => void;
  achievements: Achievement[];
  unlockedIds: string[];
}

export default function ProfileModal({ isOpen, onClose, profile, onSave, onUploadAvatar, onDeleteAvatar, achievements, unlockedIds }: ProfileModalProps) {
  ...
  const handleDeletePhoto = () => {
    if (confirm("Are you sure you want to delete your profile photo?")) {
      if (form.photoUrl && onDeleteAvatar) {
        onDeleteAvatar(form.photoUrl);
      }
          ...
          <div className="relative group">
            ...
            {form.photoUrl && (
              <button 
                ...
                disabled={isUploading}
                className="absolute bottom-0 left-0 w-8 h-8 rounded-full bg-error text-white flex items-center justify-center shadow-lg border-2 border-white z-20 hover:scale-110 transition-transform active:scale-90 disabled:opacity-50 disabled:scale-100"
              >
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setForm({ ...form, employmentType: e.target.value as StaffProfile['employmentType'] })}
                className="w-full bg-surface-container rounded-2xl py-4 px-5 outline-none border border-outline-variant/5 text-on-surface font-bold focus:border-primary/30 focus:ring-4 focus:ring-primary/5 transition-all shadow-inner"
              >
