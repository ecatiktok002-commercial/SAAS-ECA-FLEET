...
const safeFormat = (dateStr: string | null | undefined, formatStr: string) => {
  ...
  return formatInMYT(dateStr, formatStr);
};
...
export default function SignAgreement() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Determine status based on payment receipt
  ...
  const handlePrint = async () => {
    ...
    setTimeout(async () => {
      ...
      // Helper to convert URL to Base64 and get Image data
      ...
      const worker = (html2pdf() as any).set(opt).from(element).toPdf().get('pdf').then((pdf: any) => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          pdf.text(formatInMYT(getNowMYT(), 'dd/MM/yyyy'), pageWidth / 2 - 20, pageHeight - 20);
        }
      }).save();
    }, 300);
  };
    ...
    <div className="min-h-screen bg-slate-50 py-4 sm:py-12 px-0 sm:px-6 lg:px-8 font-sans text-base leading-relaxed pb-24 sm:pb-12">
      <div id="agreement-content" className="max-w-4xl mx-auto bg-white sm:shadow-xl min-h-screen sm:min-h-0 sm:rounded-2xl flex flex-col print:shadow-none print:max-w-none">
        ...
        <div className="p-3 sm:p-4 print:p-0 space-y-4 print:space-y-2 print:flex-grow print:flex print:flex-col">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 print:gap-2 print:grid-cols-2 print:page-break-inside-avoid">
            ...
            <div className="bg-slate-50 p-3 print:p-2 rounded-xl border border-slate-200 print:border-slate-300 print:rounded-none">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 print:gap-1">
                ...
                <div>
                  <p className="text-[10px] print:text-[7pt] font-bold text-slate-500 uppercase tracking-wider">Usage</p>
                  <p className="text-sm print:text-[9pt] font-medium text-slate-900">{agreement.usage || '-'}</p>
                </div>
              </div>
            </div>
          ...
          {/* 3. Section C: Terma & Syarat */}
          <section className="print:page-break-inside-avoid">
            <h2 className="text-base print:text-[9pt] font-bold text-slate-900 uppercase tracking-wide print:mb-1 border-b border-slate-200 print:border-slate-300 pb-1 print:pb-1 mb-3">Terma & Syarat Sewa Kenderaan — Ringkasan</h2>
            
            <div className="space-y-3">
              <div className="bg-white p-4 print:p-2 rounded-xl print:rounded-none border border-slate-200 print:border-black shadow-sm print:shadow-none text-xs print:text-[8pt] text-slate-700 print:text-black text-justify space-y-2">
                <p><strong>1. Penggunaan & Pemandu Sah:</strong> Kenderaan hanya untuk kegunaan persendirian yang sah. Dilarang untuk e-hailing, penghantaran, perlumbaan, aktiviti haram, sub-rental atau diserahkan/dipinjamkan kepada pihak ketiga. <strong>Hanya penyewa atau pemandu tambahan yang telah didaftarkan dan diluluskan oleh syarikat dibenarkan mengambil dan memandu kenderaan.</strong></p>
                <p><strong>2. Pengesahan Identiti:</strong> Penyewa mengesahkan bahawa semua IC, Lesen Memandu, nombor telefon, gambar, tandatangan dan maklumat yang diberikan adalah miliknya sendiri dan benar. Penyewa dilarang menyerahkan atau membenarkan orang lain menggunakan identiti, dokumen, akaun WhatsApp, OTP atau akses sewaan miliknya. Sekiranya penyewa dengan sengaja membenarkan pihak lain menggunakan identiti atau mengambil/menggunakan kenderaan bagi pihaknya, <strong>penyewa kekal bertanggungjawab bersama pihak tersebut terhadap segala kerugian, kerosakan dan tuntutan yang timbul.</strong></p>
                <p><strong>3. Self-Pickup & Penyerahan Kenderaan:</strong> Bagi self-pickup, hanya individu yang telah disahkan oleh syarikat dibenarkan mengambil kenderaan. Pengesahan melalui tandatangan digital, OTP, selfie/video, kod akses atau kaedah pengesahan syarikat dianggap sebagai pengesahan penerimaan kenderaan oleh penyewa.</p>
                <p><strong>4. Aktiviti Haram, Dadah & Sitaan Kenderaan:</strong> Dilarang menyimpan, membawa atau menggunakan kenderaan untuk dadah, barang terlarang, jenayah atau sebarang aktiviti menyalahi undang-undang. Sekiranya kenderaan ditahan atau disita akibat perbuatan penyewa atau mana-mana pihak yang mendapat akses kepada kenderaan melalui penyewa, penyewa bertanggungjawab atas <strong>kos towing, penyimpanan, pengambilan semula, kerosakan, kehilangan penggunaan, penalti dan kos munasabah berkaitan.</strong></p>
