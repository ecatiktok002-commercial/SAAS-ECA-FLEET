...
const AuditPayoutManagement: React.FC = () => {
  ...
  const [selectedAgentBookings, setSelectedAgentBookings] = useState<any | null>(null);
  ...
  const payoutSummary = useMemo(() => {
    const agentMap = new Map<string, { agent_id: string, agent_name: string, total_bookings: number, total_revenue: number, payout_due: number, records: AuditRecord[] }>();
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto font-sans">
                    ...
                    {payoutSummary.map((summary) => (
                      <tr key={summary.agent_id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-5 px-6 font-medium text-slate-600">
                          <button
                            onClick={() => setSelectedAgentBookings(summary)}
                              ...
                              {history.breakdown.map((b) => (
                                <tr key={b.agent_id}>
                                  ...
                                  <td className="py-3 px-4 text-sm text-slate-600">
                                    <button
                                      onClick={() => {
                                        let agentRecords = (b as any).records;
                                        if (!agentRecords || agentRecords.length === 0) {
                                          agentRecords = records.filter(r => {
                                            if (r.agent_id !== b.agent_id || r.payout_status !== 'paid') return false;
                                            ...
                                            if (!isValid(recordDate)) return false;
                                            return formatInMYT(recordDate, 'MMMM yyyy') === history.month_year;
                                          });
                                        }
                                        setSelectedAgentBookings({
                                      ...
                                      className="text-blue-600 hover:text-blue-800 underline underline-offset-2 transition-colors cursor-pointer"
                                    >
                                      {b.total_bookings} Bookings
                                    </button>
                                  </td>
                                  <td className="py-3 px-4 text-sm text-slate-600">RM {(Number(b.total_revenue) || 0).toFixed(2)}</td>
                                  <td className="py-3 px-4 text-right font-bold text-blue-600">RM {(Number(b.payout_due) || 0).toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
      ...
      {/* Agent Bookings Details Modal */}
      {selectedAgentBookings && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-4xl w-full overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-slate-900">{selectedAgentBookings.agent_name} - Approved Bookings</h3>
                <p className="text-sm text-slate-500 mt-1">Reviewing {selectedAgentBookings.total_bookings} bookings for this payout period.</p>
              </div>
              <button 
                onClick={() => setSelectedAgentBookings(null)}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {selectedAgentBookings.records.map((r: AuditRecord) => (
                      <tr key={r.form_id} className="hover:bg-slate-50">
                        <td className="py-3 px-4">
                          <div className="font-medium text-slate-900">{r.customer_name}</div>
                          <div className="text-xs text-slate-500">{r.reference_number || r.form_id.substring(0, 8)}</div>
                        </td>
                        <td className="py-3 px-4 font-medium text-slate-600">{r.car_plate_number}</td>
                        <td className="py-3 px-4">
                          <div className="text-sm text-slate-900">{new Date(r.form_start).toLocaleDateString()}</div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
              <div className="text-sm font-medium text-slate-600">
                Total Revenue: <span className="font-bold text-slate-900">RM {selectedAgentBookings.total_revenue.toFixed(2)}</span>
              </div>
              <button
                onClick={() => setSelectedAgentBookings(null)}
