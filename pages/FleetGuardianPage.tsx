...
const FleetGuardianPage: React.FC = () => {
  ...
  const renderStatusBadge = (status: ExpiryStatus) => (
    <div className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-tighter ${
      status.status === 'expired' ? 'text-rose-600 bg-rose-50' : 
      status.status === 'warning' ? 'text-amber-600 bg-amber-50' : 
            ...
            return (
              <div key={car.id} className="bg-white border border-slate-100 rounded-[2.5rem] p-7 hover:shadow-2xl hover:border-blue-100 transition-all group flex flex-col">
                ...
                <div className="space-y-3 mb-8">
                  ...
                  {/* PREVIOUS MONTHS LIST (Historical Data) */}
                  {history.length > 1 && (
                    <div className="px-4 py-3 bg-white border border-slate-100 rounded-2xl space-y-2 shadow-sm">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest">Recent Performance</span>
                      </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-10 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 text-center mt-3 font-medium">Historical performance and compliance data will be lost.</p>
            <div className="flex flex-col gap-3 mt-8">
              <button onClick={() => handleDeleteCar(showDeleteConfirm)} className="w-full py-4 bg-rose-600 text-white rounded-2xl font-black uppercase text-xs hover:bg-rose-700 shadow-lg">Confirm Delete</button>
