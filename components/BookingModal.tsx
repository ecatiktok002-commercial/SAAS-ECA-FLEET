...
const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const [bookingMode, setBookingMode] = useState<"category" | "specific">(
    "category",
  );
  ...
  const [pendingAction, setPendingAction] = useState<{
    type: "save" | "delete" | "delete_handover" | "delete_handover_photo";
    data?: any;
  } | null>(null);
  ...
  useEffect(() => {
    if (
      viewingRecord &&
      viewingRecord.photos_url &&
      viewingRecord.photos_url.length > 0
    ) {
  ...
  useEffect(() => {
    if (editingBooking && subscriberId) {
      ...
      apiService
        .getLinkedAgreementForBooking(editingBooking.id, subscriberId)
        .then(setLinkedAgreement)
        .catch(() => setLinkedAgreement(null));
    } else {
      setHandoverRecords([]);
  ...
  const modelAvailability = useMemo(() => {
    ...
    uniqueModels.forEach((model) => {
      const modelCars = cars.filter(
        (c) => c.name.trim() === model && c.status === "active",
      );
      const available = modelCars.filter((car) => {
        ...
        const bookingData = {
          ...
          ...(isEarlyReturn && actualEndTime
            ? { actual_end_time: mytToUtc(actualEndTime).toISOString() }
    ...
    if (isOpen) {
      ...
      if (editingBooking) {
        ...
        if (editingBooking.actual_end_time) {
          ...
          editingBooking.end_time.includes("-")
        ) {
  ...
  const isSelectedModelFull =
    selectedModel && modelAvailability[selectedModel] === 0;
  ...
  const isEditable = useMemo(() => {
    ...
    const isSuperAdmin =
      ...
      (staffRole === "admin" && currentUserId === "superadmin");
    ...
    if (editingBooking.created_by) {
      const canEdit =
        editingBooking.created_by === currentUserId ||
        editingBooking.created_by === userUid;
      return canEdit;
    }

    // Fallback for legacy bookings without created_by
    const isAgent =
      editingBooking.agent_id === currentUserId ||
      editingBooking.agent_id === userUid;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedDateTimeStr)
      return setError("Please select a start date and time");
    ...
    if (isEarlyReturn) {
      ...
      if (
        ...
        new Date(selectedDateTimeStr).getTime()
      ) {
    ...
    if (bookingMode === "category") {
      ...
      if (!foundCarId) {
        ...
        if (upgrade) {
          ...
          setError(
            `No ${selectedModel} available. Upgrade available: ${upgrade.name} (${upgrade.type}).`,
          );
        } else {
          setError(
            `All ${selectedModel}s are fully booked for these dates, and no upgrades are available.`,
          );
        }
        return;
      }
    ...
    if (bookingMode === "specific") {
      ...
      if (!validateBooking(bookingData, otherBookings)) {
        return setError(
          "Conflict detected! This specific vehicle is busy during the requested time window.",
        );
      }
    }
    ...
    if (editingBooking && subscriberId) {
      ...
      if (!selectedMember.is_subscriber) {
        try {
          const staff = await apiService.getStaffMemberByName(
            finalStaffName,
            subscriberId,
          );
  ...
  const handlePinSuccess = () => {
    if (pendingAction && selectedStaffMember) {
      if (pendingAction.type === "save") {
        ...
        deleteHandoverPhoto(
          pendingAction.data.recordId,
          pendingAction.data.photoIndex,
        );
      }
    }
      ...
      if (!selectedMember.is_subscriber) {
        try {
          ...
          if (staff && staff.pin_hash) {
            setSelectedStaffMember(staff);
            setPendingAction({
              type: "delete_handover_photo",
              data: { recordId: viewingRecord.id, photoIndex: index },
            });
            setIsPinModalOpen(true);
            return;
          }
  ...
  const deleteHandoverPhoto = async (recordId: string, photoIndex: number) => {
    try {
      ...
      // Update local state
      const updatedPhotos = viewingRecord.photos_url.filter(
    ...
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                <div className="w-8 h-8 bg-amber-100 rounded-full flex items-center justify-center text-amber-600 shrink-0">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth="2"
                      d="M12 15v2m0 0v2m0-2h2m-2 0H10m11-3V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2h14a2 2 0 002-2v-4z"
                    />
                  </svg>
                </div>
                <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wider leading-tight">
                  Privacy Shield Active: This booking belongs to another agent
            ...
            {editingBooking && isEditable && (
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                ...
                <p className="text-[9px] text-slate-400 text-center leading-tight">
                  Send these links to the customer via WhatsApp for self-service
                  handover.
                </p>
              </div>
            )}
            ...
            {bookingMode === "category" ? (
              <div>
                    ...
                    {uniqueModels.map((model) => {
                      ...
                      return (
                        <option
                          key={model}
                          value={model}
                          className={isFull ? "text-rose-500 font-bold" : ""}
                        >
                          {label}
                        </option>
                      );
                ...
                <p
                  className={`text-[10px] mt-2 transition-colors ${isSelectedModelFull ? "text-rose-500 font-bold" : "text-slate-400"}`}
                >
                  {isSelectedModelFull
                    ? "This model is unavailable for the selected dates."
                    : "System will automatically assign the best available plate for this model."}
                </p>
              </div>
            ) : (
              <div>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                {bookingMode === "specific" && (
                  <div className="flex items-center gap-2">
                    ...
                    <label
                      htmlFor="earlyReturn"
                      className="text-[10px] font-bold text-blue-600 uppercase tracking-wider cursor-pointer select-none hover:text-blue-800 disabled:opacity-60"
                    >
            ...
            {error && (
              <div
                className={`p-3 border rounded-xl text-xs font-bold flex flex-col gap-2 ${upgradeSuggestion ? "bg-amber-50 border-amber-100 text-amber-700" : "bg-rose-50 border-rose-100 text-rose-600"}`}
              >
                <div className="flex items-center gap-2">
                  <svg
              ...
              {editingBooking &&
                ...
                (subscriptionTier === "tier_2" ? (
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/forms/edit/${linkedAgreement.id || linkedAgreement.form_id}`,
                      )
                    }
                    ...
                    onClick={async () => {
                      if (subscriptionTier === "tier_3" && subscriberId) {
                        ...
                        try {
                          ...
                          await supabase.rpc("run_heuristic_match", {
                            target_subscriber_id: subscriberId,
                          });
                          const match =
                            await apiService.getLinkedAgreementForBooking(
                              editingBooking.id,
                              subscriberId,
                            );
                          if (match) {
                            ...
                            toast.success(
                              "Digital Form Automatically Matched!",
                            );
              ...
              {handoverRecords.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-slate-100 mt-2">
                  ...
                  {handoverRecords.map((record) => {
                    const handoverAgent = members.find(
                      (m) => m.staff_id === record.staff_id,
                    );
                    return (
                      <div
                        key={record.id}
        ...
        {isHandoverOpen && editingBooking && subscriberId && (
          <HandoverForm
            ...
            vehiclePlate={
              cars.find((c) => c.id === editingBooking.car_id)?.plate ||
              "Unknown Vehicle"
            }
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-lg">
                    {viewingRecord.handover_type} Record
                  </h3>
                  {viewingRecord.staff_id ? (
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-300 rounded text-[10px] font-bold uppercase tracking-wider border border-blue-500/30">
                      {members.find(
                        (m) => m.staff_id === viewingRecord.staff_id,
                      )?.name || "Unknown Agent"}
                    </span>
                  ) : (
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {formatInMYT(
                    new Date(viewingRecord.created_at).getTime(),
                    "dd/MM/yyyy HH:mm",
                  )}
                </p>
              </div>
                  ...
                  {signedUrls.map((url: string, index: number) => (
                    ...
                    >
                      <div className="relative aspect-square rounded-xl overflow-hidden bg-slate-900 border border-slate-800 hover:border-blue-500 transition-colors">
                        ...
                        <p className="absolute bottom-2 left-2 text-white text-[10px] font-bold uppercase tracking-wider">
                          Photo {index + 1}
                        </p>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white font-bold">
                          {viewingRecord.mileage} km
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Fuel Level:</span>
                        <span className="text-white font-bold">
                          {viewingRecord.fuel_level}
                        </span>
                      </div>
                    </div>
      ...
      <PinModal
        ...
        title={
          ...
          pendingAction?.type === "delete_handover" ||
          pendingAction?.type === "delete_handover_photo"
            ? "Confirm Deletion"
            : "Confirm Update"
        }
