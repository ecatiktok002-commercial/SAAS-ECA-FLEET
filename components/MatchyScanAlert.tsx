...
interface MatchyScanAlertProps {
  ...
  scanTrigger?: number;
  onScanComplete?: () => void;
  orphanedAgreements?: any[];
}

const MatchyScanAlert: React.FC<MatchyScanAlertProps> = ({ 
  subscriberId, 
  monthStartDate, 
  monthEndDate, 
  scanTrigger = 0, 
  onScanComplete,
  orphanedAgreements: propOrphanedAgreements
}) => {
  ...
  const displayAgreements = propOrphanedAgreements !== undefined
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <h3 className="text-emerald-800 font-bold text-lg tracking-tight">
                Data Integrity Verified: Ready for Payout
              </h3>
              <button 
            ...
            <p className="text-emerald-600 text-sm mt-1 font-medium">
              Excellent work! All Digital Forms have been fully audited and matched with Calendar Bookings for this period. You may now proceed to review and approve payouts without any data integrity issues.
            </p>
          </div>
        </div>
      );
      ...
      <div className="space-y-6 mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <div className="flex-1">
          ...
          <p className="text-rose-600 text-sm mt-1 font-medium">
            The Matchy Scan detected {displayAgreements.length} orphaned agreement(s) for this month. You must resolve these discrepancies before proceeding to Bank Reconciliation.
          </p>
          <div className="flex flex-wrap gap-4 mt-3">
            {displayAgreements.length > 0 && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-100 text-rose-700 text-xs font-bold uppercase tracking-wider rounded-md">
                ⚠️ {displayAgreements.length} Agreements missing on Calendar
              </span>
            )}
          </div>
        </div>
